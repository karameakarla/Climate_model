...
-- Generar el CROSS APPLY dinámico
---------------------------------------------------
...
--  Construimos la lista dinámica de columnas para UNPIVOT
----------------------------------------------------------
...
--  Verificamos que haya columnas válidas
...
-- Armamos el SQL dinámico que hace el UNPIVOT
SET @sql = N'
...
--  (Opcional) Muestra lo que se va a ejecutar
PRINT @sql;
...
---CONSULTAS PREVIA A MODIFICAR taBLA INDICADOR (2025/10/23)
SELECT 
    indicador_nombre,
    COUNT(*) AS cantidad_registros
FROM indicador
GROUP BY indicador_nombre
HAVING COUNT(*) > 1
ORDER BY cantidad_registros DESC;

--Consulta 2 — Duplicados por indicador_nombre y país
SELECT 
    i.indicador_nombre,
    p.pais_nombre,
...
GROUP BY i.indicador_nombre, p.pais_nombre
HAVING COUNT(*) > 1
ORDER BY veces_repetido DESC;

--Consulta 3 — Comparar IDs y fuentes entre duplicados
SELECT 
    i.indicador_nombre,
    p.pais_nombre,
    i.pais_id,
    ...
    STRING_AGG(CAST(i.id AS VARCHAR(10)), ', ') AS ids_repetidos
FROM indicador i
JOIN pais p ON i.pais_id = p.id
GROUP BY i.indicador_nombre, p.pais_nombre, i.pais_id, i.fuente_id, i.evento_extremo_id
HAVING COUNT(*) > 1
ORDER BY repeticiones DESC;


--🔸 Paso 1: Identificar los duplicados que se eliminarán
SELECT id
FROM indicador
WHERE id NOT IN (
    SELECT MIN(id)
    FROM indicador
    GROUP BY pais_id, fuente_id, evento_extremo_id, indicador_nombre
)
ORDER BY id;


--   Paso 2 — Crea la copia temporal (respaldo lógico) -----(2025/10/24)-----
-- [A] Crear respaldo temporal con duplicados detectados
SELECT *
INTO indicador_backup_duplicados
FROM indicador
WHERE id IN (
    SELECT id
    FROM indicador
...
-- Paso 3  - Comprobar cuantos duplicados quedaron
SELECT COUNT(*) AS duplicados_guardados FROM indicador_backup_duplicados;


-- Calcular el id “canonical” a conservar por cada grupo
WITH ranked AS (
    SELECT 
        i.*,
        ROW_NUMBER() OVER (
            PARTITION BY i.pais_id, i.fuente_id, i.evento_extremo_id, i.indicador_nombre
            ORDER BY 
               CASE WHEN i.evento_extremo_id IS NOT NULL THEN 0 ELSE 1 END, -- prioriza filas con evento_extremo_id no null
               i.id   -- si empate, el id más pequeño
        ) AS rn
    FROM indicador i
)
-- Ver qué filas serán consideradas "a conservar" (rn = 1)
SELECT *
FROM ranked
WHERE rn = 1
ORDER BY indicador_nombre, pais_id;


-- Previsualizar qué ids se eliminarían (sólo verificación)
WITH ranked AS (
    SELECT 
        i.*,
        ROW_NUMBER() OVER (
            PARTITION BY i.pais_id, i.fuente_id, i.evento_extremo_id, i.indicador_nombre
            ORDER BY 
               CASE WHEN i.evento_extremo_id IS NOT NULL THEN 0 ELSE 1 END,
               i.id
        ) AS rn
    FROM indicador i
)
SELECT id, indicador_nombre, pais_id, fuente_id, evento_extremo_id
FROM ranked
WHERE rn > 1
ORDER BY indicador_nombre, pais_id, id;


EXEC sp_help 'indicador';
EXEC sp_help 'registro_indicador';
sp_help indicador
EXEC sp_help 'dbo.registro_indicador';
SELECT*  FROM indicador;


-- Motrar duplicados (2025/10/24)
;WITH duplicados AS (
    SELECT 
        indicador_nombre,
        pais_id,
    ...
    FROM indicador
    GROUP BY indicador_nombre, pais_id, fuente_id, evento_extremo_id
    HAVING COUNT(*) > 1
),
a_eliminar AS (
    SELECT i.id
    ...
    INNER JOIN duplicados d
        ON i.indicador_nombre = d.indicador_nombre
        AND i.pais_id = d.pais_id
        AND i.fuente_id = d.fuente_id
        AND i.evento_extremo_id = d.evento_extremo_id
        AND i.id <> d.id_conservar
)
SELECT i.id, i.indicador_nombre, i.pais_id, i.fuente_id, i.evento_extremo_id
FROM indicador i
WHERE i.id IN (SELECT id FROM a_eliminar);



-- Paso 4 -  Limpiar duplicados (Paso seguro)
BEGIN TRAN;

DELETE FROM indicador
WHERE id IN (55, 65, 76, 85);


-- Verifica que solo quedan los correctos
SELECT * 
FROM indicador
WHERE indicador_nombre IN ('Sequía prolongada', 'Sequía agrícola')
ORDER BY indicador_nombre, id;

COMMIT TRAN;
ROLLBACK TRAN; (para eliminar cualquier cambio)
...
--Recomendación final (evitar duplicados en el futuro)
ALTER TABLE indicador
ADD CONSTRAINT UQ_indicador_unico
UNIQUE (pais_id, fuente_id, evento_extremo_id, indicador_nombre);


--Ejercicios 
--EJERCICIO 1 — Verificar relaciones básicas (JOIN entre tablas principales)
SELECT 
    p.pais_nombre,
    f.fuente_nombre,
    i.indicador_nombre,
...
FROM indicador i
JOIN pais p ON i.pais_id = p.id
JOIN fuente_datos f ON i.fuente_id = f.id
ORDER BY p.pais_nombre;



--EJERCICIO 2 — Comprobar vínculos entre eventos extremos e indicadores




--EJERCICIO 3 — Verificar datos históricos de CO₂ (registro_indicador + fuente)
SELECT 
    r.indicador_nombre,
    r.periodo,
    r.valor,
...
JOIN fuente_datos f ON r.fuente_id = f.id
WHERE r.indicador_nombre LIKE '%CO2%'
ORDER BY p.pais_nombre, r.periodo;


--EJERCICIO 4 — Cruce entre desviaciones, eventos e indicadores
SELECT 
    d.id AS desviacion_id,
    p.pais_nombre,
    ee.tipo_evento,
    d.tipo_desviacion,
    d.valor_calculado,
...
JOIN registro_indicador r ON d.registro_indicador_id = r.id
JOIN pais p ON r.pais_id = p.id
LEFT JOIN evento_extremo ee ON d.evento_extremo_id = ee.id
ORDER BY p.pais_nombre;


--EJERCICIO 5 — Validar trazabilidad completa (flujo total de datos)
SELECT 
    p.pais_nombre,
    ee.tipo_evento,
    ee.ubicacion,
    ...
    f.fuente_nombre
FROM pais p
LEFT JOIN evento_extremo ee ON ee.pais_id = p.id
LEFT JOIN indicador i ON i.evento_extremo_id = ee.id
LEFT JOIN registro_indicador r ON r.pais_id = p.id AND r.indicador_nombre = i.indicador_nombre
LEFT JOIN desviaciones_indicadores d ON d.registro_indicador_id = r.id
LEFT JOIN fuente_datos f ON i.fuente_id = f.id
ORDER BY p.pais_nombre;


-- Paso 5 - Limpiar los demas duplicados (2025-25-10)
BEGIN TRAN;

--DELETE FROM indicador (Verifique que elimina todo)
WHERE id IN (36, 38, 25, 45, 63, 75, 84, 22, 42, 60, 72, 77, 81, 44, 30, 33, 62, 74, 83, 21, 41, 59, 71, 79);


--BEGIN TRAN;
--DELETE FROM indicador solo de suriname (Verifique que elimina 6 de 7 )
--WHERE id IN (38, 25, 45, 63, 75, 84)


--BEGIN TRAN; (2do en probar)

DELETE FROM indicador
WHERE id IN (42, 60, 72, 77, 81, 44, 33, 62, 74, 83, 41, 59, 71, 79);

--BEGIN TRAN; (3ro en probar)
BEGIN TRAN
DELETE FROM indicador
WHERE id IN (82, 73, 61, 43, 40, 70, 58, 78, 66, 56, 47, 64);

--BEGIN TRAN; (4to en probar)
BEGIN TRAN
DELETE FROM indicador
WHERE id IN (48, 49, 68, 39, 51, 46, 52);


--BEGIN TRAN; (1ro en probar)
...
--  Confirmar la eliminacion
SELECT * FROM indicador
WHERE indicador_nombre IN ('ciclon Catarina')
ORDER BY indicador_nombre, id;

--Verificar si aun existen dupluicados
SELECT pais_id, fuente_id, evento_extremo_id, indicador_nombre, COUNT(*) AS repeticiones
FROM indicador
GROUP BY pais_id, fuente_id, evento_extremo_id, indicador_nombre
HAVING COUNT(*) > 1;
...
---A--LIMPIAR TABLA INDICADOR ( inconsistencias 22-10-2025)
--DELETE FROM indicador;

---B--RELLENAR con el SEED ORIGINAL
--Alterar colum evento_extremo_id
ALTER TABLE indicador
...
-- SEED DATA: Indicador 
-- ========================
--Indicadores vinculados a Eventos extremos ===total 14
...
-- ========================
--Indicadores historicos-------------------VERIFICAR SI ES NECESARIO COLOCARLA PORQUE EXISTE TABLA CO2
...
VALUES(
 ...
 SELECT TOP 50* FROM registro_indicador
...
-------------Esto ya no se considera 2025-10-22 existe dboCO2
...
---------------------EJERCIOS DE VALIACION ---VERIFICAR SI SE LA PONE, PORQUE SE USO dboCO2--------- 
...
--Para consultar lo que existe en la tabla--
...
----evento_extermo tiene 18 NULL POR AHORA
SELECT id, tipo_evento, ubicacion, fecha, fuente_id
FROM dbo.evento_extremo
WHERE fuente_id IS NULL;


--EVENTO_EXTREMO TIPO EVENTO TIENE 10 REGISTROS POR AHORA
...
----importante DATOOOOOOOOOOOOOOOOOOOOOOO
...
--SSALIO NO BIEN 
--INDICADOR  AQUI EXISTEN 15 SIN CONTAR LOS 12 DE CO2
SELECT DISTINCT indicador_nombre
FROM registro_indicador
ORDER BY indicador_nombre;

--EVENTO _EXTREMO --EXISTEN 10 SIN CONTAR CON CO2 
SELECT DISTINCT tipo_evento
FROM evento_extremo
ORDER BY tipo_evento;

sp_help registro_indicador;
SELECT TOP 80 * FROM registro_indicador;
SELECT TOP 80 * FROM indicador;
sp_help desviaciones_indicadores;
SELECT TOP 40 * FROM desviaciones_indicadores;
...
----------DESDE AQUI VERIFICAR DESVIACIONES------------
...
WHERE d.tipo_desviacion LIKE '%Pedernales%'
...
SELECT COUNT(*) FROM desviaciones_indicadores;
SELECT COUNT(*) FROM registro_indicador;
SELECT COUNT(*) 
FROM desviaciones_indicadores d
LEFT JOIN registro_indicador r ON d.registro_indicador_id = r.id
WHERE r.id IS NULL;


SELECT DISTINCT indicador_nombre FROM registro_indicador WHERE pais_id = (SELECT id FROM pais WHERE pais_nombre = 'Paraguay');

SELECT TOP 50 * 
FROM registro_indicador 
WHERE indicador_nombre LIKE '%Inundacion%' 
  --AND pais_id = (SELECT id FROM pais WHERE pais_nombre = 'peru');

SELECT TOP 3 d.*, r.indicador_nombre, r.pais_id 
FROM desviaciones_indicadores d
JOIN registro_indicador r ON d.registro_indicador_id = r.id;

SELECT TOP 10 d.*, ee.tipo_evento 
FROM desviaciones_indicadores d
JOIN evento_extremo ee ON d.evento_extremo_id = ee.id
WHERE tipo_desviacion = 'Mw';




sp_help desviaciones_indicadores;( CALCULA VALOR- UNID RESULTADO)
sp_help evento_extremo;(TIENE VALOR)
sp_help registro_indicador; (TIENE VALOR E INDICADOR NOMBRE)
sp_help indicador; (LIBRERIA SOLO UNIDAD E INDICADOR NOMBRE)

--Insertar un evento extremo de tipo inundación para Paraguay
INSERT INTO evento_extremo (pais_id, tipo_evento, descripcion, fecha, ubicacion)
VALUES (
    (SELECT id FROM pais WHERE pais_nombre = 'Paraguay'),
    'Inundación',
    'Inundaciones por crecida del río Paraguay - evento histórico base',
    '2022-03-01',
...
SELECT * 
FROM evento_extremo 
WHERE tipo_evento LIKE '%Inundación%' 
  AND pais_id = (SELECT id FROM pais WHERE pais_nombre = 'Paraguay');

  --Verificar 
SELECT * 
FROM registro_indicador 
WHERE indicador_nombre LIKE '%terremoto%' 
  AND pais_id = (SELECT id FROM pais WHERE pais_nombre = 'ecuador');
...
LEFT JOIN pais_alias a
  ...
  /*
-- Script de eventos base por país (ya no se utiliza)
-- Fue verificado: no existen registros activos.
-- Se mantiene como referencia histórica.
=============================================
-- 🌎 EVENTOS EXTREMOS BASE POR PAÍS
-- =============================================
...
SELECT p.id, 
       'Inundación', 
       CONCAT('Evento base de inundación en ', p.pais_nombre), 
       '2022-01-15', 
       p.pais_nombre
FROM pais p
WHERE p.id NOT IN (
    SELECT DISTINCT pais_id FROM evento_extremo WHERE tipo_evento = 'Inundación'
);
...
SELECT p.id, 
       'Sequía', 
       CONCAT('Evento base de sequía prolongada en ', p.pais_nombre), 
       '2022-02-01', 
       p.pais_nombre
FROM pais p
WHERE p.id NOT IN (
    SELECT DISTINCT pais_id FROM evento_extremo WHERE tipo_evento = 'Sequía'
);

-- Incendio forestal (12)
INSERT INTO evento_extremo (pais_id, tipo_evento, descripcion, fecha, ubicacion)
SELECT p.id, 
       'Incendio forestal', 
       CONCAT('Evento base de incendio forestal en ', p.pais_nombre), 
       '2022-03-01', 
       p.pais_nombre
FROM pais p
WHERE p.id NOT IN (
    SELECT DISTINCT pais_id FROM evento_extremo WHERE tipo_evento = 'Incendio forestal'
);
...
SELECT p.id, 
       'Ola de calor', 
       CONCAT('Evento base de ola de calor en ', p.pais_nombre), 
       '2022-04-01', 
       p.pais_nombre
FROM pais p
WHERE p.id NOT IN (
    SELECT DISTINCT pais_id FROM evento_extremo WHERE tipo_evento = 'Ola de calor'
);
...
SELECT p.id, 
       'Terremoto', 
       CONCAT('Evento base de terremoto registrado en ', p.pais_nombre), 
       '2022-05-01', 
       p.pais_nombre
FROM pais p
WHERE p.id NOT IN (
    SELECT DISTINCT pais_id FROM evento_extremo WHERE tipo_evento = 'Terremoto'
);


SELECT COUNT(*) AS total_paises FROM pais;
SELECT COUNT(*) AS total_eventos FROM evento_extremo;

SELECT tipo_evento, COUNT(DISTINCT pais_id) AS paises_con_evento
FROM evento_extremo
GROUP BY tipo_evento
ORDER BY tipo_evento;

-- Ejemplo para Inundación
SELECT p.id, p.pais_nombre
FROM pais p
WHERE p.id NOT IN (
    SELECT DISTINCT pais_id 
    FROM evento_extremo 
    WHERE tipo_evento = 'Inundación'
)
ORDER BY p.pais_nombre;

--esta mal
SELECT e.id, p.pais_nombre, e.tipo_evento, e.descripcion, e.fecha, e.ubicacion
FROM evento_extremo e
JOIN pais p ON e.pais_id = p.id
ORDER BY p.pais_nombre, e.tipo_evento;


-- 1. Previsualizar los eventos "base" que generamos (sale vacio)
SELECT e.id, p.pais_nombre, e.tipo_evento, e.descripcion, e.fecha, e.ubicacion
FROM evento_extremo e
JOIN pais p ON e.pais_id = p.id
WHERE e.descripcion LIKE 'Evento base de %'
ORDER BY p.pais_nombre, e.tipo_evento;

-- 2.a. Eventos que tienen indicadores referenciándolos
SELECT i.id AS indicador_id, i.indicador_nombre, i.pais_id, i.evento_extremo_id
FROM indicador i
WHERE i.evento_extremo_id IN (
    SELECT e.id FROM evento_extremo e WHERE e.descripcion LIKE 'Evento base de %'
);

-- 2.b. Si tienes otras tablas que podrían referenciar evento_extremo, revisa también:
-- ejemplo: desviaciones_indicadores
SELECT d.id AS desviacion_id, d.evento_extremo_id
FROM desviaciones_indicadores d
WHERE d.evento_extremo_id IN (
    SELECT e.id FROM evento_extremo e WHERE e.descripcion LIKE 'Evento base de %'
); */ 

--confirmar que eventos_extremo igure condatos intactos
SELECT COUNT(*) AS total_eventos, COUNT(DISTINCT pais_id) AS paises_con_eventos
FROM evento_extremo;

SELECT COUNT(*) AS indicadores_con_evento_valido
FROM indicador i
WHERE i.evento_extremo_id IS NOT NULL
  AND i.evento_extremo_id IN (SELECT id FROM evento_extremo);


  sp_help evento_extremo;
--1️⃣ Consulta general: conteo por país y nombre de evento
SELECT 
    p.pais_nombre,
    e.tipo_evento,
    COUNT(*) AS cantidad
FROM evento_extremo e
JOIN pais p ON e.pais_id = p.id
GROUP BY p.pais_nombre, e.tipo_evento
HAVING COUNT(*) > 1
ORDER BY p.pais_nombre, cantidad DESC;


--2️⃣ Verificar cuántos eventos únicos reales tienes
SELECT 
    p.pais_nombre,
    COUNT(DISTINCT e.tipo_evento) AS eventos_unicos
FROM evento_extremo e
JOIN pais p ON e.pais_id = p.id
GROUP BY p.pais_nombre
ORDER BY eventos_unicos DESC;

SELECT TOP 60 * FROM evento_extremo;


--Limpiar sin perder datos reales

SELECT * INTO evento_extremo_backup_20251026
FROM evento_extremo;
...
--- A. Detectar los duplicados sin borrar nada
SELECT 
    p.pais_nombre,
    e.tipo_evento,
    e.ubicacion,
    STRING_AGG(CONVERT(varchar(10), e.id), ', ') AS ids_duplicados,
    COUNT(*) AS cantidad
FROM evento_extremo e
JOIN pais p ON e.pais_id = p.id
GROUP BY p.pais_nombre, e.tipo_evento, e.ubicacion
HAVING COUNT(*) > 1
ORDER BY p.pais_nombre;



--- B. Eliminar duplicados (con seguridad)
BEGIN TRAN;
ROLLBACK TRAN;



--- C. Script para ver las dependencias fk con indicador
SELECT 
    ee.id AS evento_id,
    ee.tipo_evento,
    ee.ubicacion,
    COUNT(i.id) AS indicadores_relacionados
FROM evento_extremo ee
LEFT JOIN indicador i ON ee.id = i.evento_extremo_id
GROUP BY ee.id, ee.tipo_evento, ee.ubicacion
ORDER BY indicadores_relacionados DESC, ee.id;
...
SELECT id, tipo_evento, ubicacion
FROM evento_extremo
WHERE id > 14;

-- 🗑️ Elimina los eventos que no tienen referencias
--DELETE FROM evento_extremo
--WHERE id = 15;

-- ✅ Verifica que solo queden los primeros 16 OJO QUE EL 16 ES REPETIDO Y EL 15 ELIMINAR LUEGOI
SELECT COUNT(*) AS total_eventos_restantes FROM evento_extremo;
SELECT * FROM evento_extremo ORDER BY id;
ROLLBACK TRAN;
...
sp_help indicador;
-- indicadores dependen de eventos con id > 14:
SELECT 
    ...
    e.ubicacion
FROM indicador i
JOIN evento_extremo e ON i.evento_extremo_id = e.id
WHERE e.id > 14
ORDER BY e.id;


--cuántos registros están vinculados:
SELECT COUNT(*) AS total_bloqueados
FROM indicador
WHERE evento_extremo_id > 14;

--Verifica el ID del evento original (válido) para “Tropical cyclone – Santa Catarina, Brazil”
SELECT id, tipo_evento, ubicacion
FROM evento_extremo
WHERE tipo_evento = 'Tropical cyclone'
  AND ubicacion = 'Santa Catarina, Brazil';


-- cual indicador depende de este evento 
SELECT i.id AS indicador_id,
       ...
       e.ubicacion
FROM indicador i
JOIN evento_extremo e
  ON i.evento_extremo_id = e.id
WHERE e.tipo_evento = 'Tropical cyclone'
  AND e.ubicacion = 'Santa Catarina, Brazil'
ORDER BY i.evento_extremo_id;


-- 2️⃣ Reasignar todos los indicadores duplicados al evento principal (id=2)
UPDATE indicador
SET evento_extremo_id = 2
WHERE evento_extremo_id IN (16);
...
sp_help evento_extremo;
--evitar duplicados para evento extremo con condicionews



--Restriccion unica para evento_extremo evita duplicados
ALTER TABLE evento_extremo
ADD CONSTRAINT UQ_evento_unico 
UNIQUE (pais_id, tipo_evento, ubicacion, fecha);


sp_help registro_indicador;
sp_help indicador;

--Correlacionar con evento_extremo
ALTER TABLE registro_indicador
ADD evento_extremo_id INT;

ALTER TABLE registro_indicador
ADD CONSTRAINT FK_registro_evento
FOREIGN KEY (evento_extremo_id)
REFERENCES evento_extremo(id);


--update corregido
-- 2️⃣ Actualizamos los registros vinculando por país e indicador_nombre
--    Ejemplo: asociamos eventos de sequía a registros con indicadores de sequía.
UPDATE ri
SET ri.evento_extremo_id = e.id
FROM registro_indicador ri
JOIN evento_extremo e 
    ON ri.pais_id = e.pais_id
WHERE (
        (ri.indicador_nombre LIKE '%sequía%' AND e.tipo_evento LIKE '%sequía%') OR
        (ri.indicador_nombre LIKE '%inund%' AND e.tipo_evento LIKE '%inund%') OR
        (ri.indicador_nombre LIKE '%temperatura%' AND e.tipo_evento LIKE '%calor%') OR
        (ri.indicador_nombre LIKE '%precipit%' AND e.tipo_evento LIKE '%precipit%') OR
        (ri.indicador_nombre LIKE '%CO2%' AND e.tipo_evento LIKE '%ciclón%')
      );

SELECT TOP 20* FROM indicador
...
SELECT DISTINCT indicador_nombre FROM registro_indicador;
SELECT DISTINCT tipo_evento, descripcion FROM evento_extremo;




-- =====================================
-- 1️⃣ Crear tabla temporal de mapeo manual
-- =====================================
CREATE TABLE #mapa_eventos (
    tipo_evento NVARCHAR(150),
    indicador_nombre NVARCHAR(150)
);

INSERT INTO #mapa_eventos (tipo_evento, indicador_nombre)
VALUES
('Tropical cyclone', 'Ciclón Catarina'),
('Fenómeno climático global', 'Deslizamiento Huaycoloro'),
('Incendios forestales', 'Incendios amazónicos'),
('Continental weather and climate extremes', 'Inundaciones en Georgetown'),
('Precipitaciones extremas', 'Inundaciones La Niña'),
('Continental weather and climate extremes', 'Inundaciones litoral uruguayo'),
('Continental weather and climate extremes', 'Inundaciones por crecida del río Paraguay'),
('Continental weather and climate extremes', 'Inundaciones Suriname'),
('Peligro biologico', 'Plaga de langostas'),
('Sequía', 'Sequía agrícola'),
('Aridity', 'Sequía extrema de Arica'),
('Sequía', 'Sequía prolongada'),
('Terremoto', 'Terremoto Pedernales'),
('Continental weather and climate extremes', 'Tragedia de Vargas');


--Actualizar los vínculos (evento_extremo_id)
BEGIN TRAN;

UPDATE ri
SET ri.evento_extremo_id = e.id
FROM dbo.registro_indicador AS ri
INNER JOIN #mapa_eventos AS m 
    ON ri.indicador_nombre = m.indicador_nombre
INNER JOIN dbo.evento_extremo AS e
    ON e.tipo_evento = m.tipo_evento;

-- Verificar resultados
SELECT 
    ri.id,
    ri.indicador_nombre,
    ri.evento_extremo_id,
    e.tipo_evento,
    e.descripcion
FROM dbo.registro_indicador AS ri
LEFT JOIN dbo.evento_extremo AS e ON ri.evento_extremo_id = e.id
WHERE ri.evento_extremo_id IS NOT NULL;

COMMIT TRAN;

ROLLBACK TRAN;
DROP TABLE #mapa_eventos;

SELECT DISTINCT indicador_nombre FROM registro_indicador ORDER BY indicador_nombre;
SELECT DISTINCT tipo_evento FROM evento_extremo ORDER BY tipo_evento;
...
---2025-10-28
SELECT COUNT(*) AS total_desviaciones
...
--Opción A: Dejar las filas, pero mostrar solo una por país–indicador–fecha
SELECT
    p.pais_nombre,
    ri.indicador_nombre,
    MAX(di.valor_calculado) AS valor_calculado,
    MAX(di.clasificacion_alertas) AS clasificacion_alertas,
    MAX(di.metodologia) AS metodologia,
    MAX(ri.fecha) AS fecha
FROM desviaciones_indicadores di
JOIN registro_indicador ri ON di.registro_indicador_id = ri.id
LEFT JOIN pais p ON ri.pais_id = p.id
WHERE ri.indicador_nombre = 'Emisiones de CO2 históricas'
GROUP BY p.pais_nombre, ri.indicador_nombre;

--versión de todos los indicadores, incluso sin datos aún (rellenando con NULL los campos de desviaciones):
SELECT
    p.pais_nombre,
    ri.indicador_nombre,
    MAX(di.valor_calculado) AS valor_calculado,
    ...
    MAX(ri.fecha) AS fecha
FROM registro_indicador ri
LEFT JOIN desviaciones_indicadores di ON di.registro_indicador_id = ri.id
LEFT JOIN pais p ON ri.pais_id = p.id
GROUP BY p.pais_nombre, ri.indicador_nombre
ORDER BY p.pais_nombre, ri.indicador_nombre;



--Script para crear la vista consolidada
CREATE OR ALTER VIEW vw_desviaciones_resumen AS
SELECT
    p.pais_nombre,
    ri.indicador_nombre,
    MAX(di.valor_calculado) AS valor_calculado,
...
--consulta de vista ordenada
SELECT * 
FROM vw_desviaciones_resumen
ORDER BY pais_nombre, indicador_nombre;
